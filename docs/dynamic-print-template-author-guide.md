...
第一次编写模板时，先跑通“模板集合”中的完整示例，再阅读标签、页面、数据路径和格式化章节。需要片段复用时看 include，需要目录、跳转或批注时看导航语义；上线前最后对照“发布前检查”和[验证与验收指南](dynamic-print-verification-guide.md)。

```mermaid
flowchart LR
    A[宿主准备标准 JSON] --> B[发布完整文档与片段]
    B --> C[编译并做静态检查]
    C --> D[锁定模板版本快照]
    D --> E[绑定字段、条件和循环]
    E --> F[检查真实文档能力]
...
模板作者只负责从“发布完整文档与片段”到“绑定字段、条件和循环”的声明。数据库查询、字典、脱敏、权限和交付流程应在进入框架前后由宿主完成。
