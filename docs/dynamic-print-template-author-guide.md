...
这份示例覆盖 7C 的完整编译与绑定语义。当前内置 PDF 对页眉页脚、命名样式和多页面序列仍会明确报告能力不足；这些高级映射在 7D 完成前不会被静默忽略。只使用基础正文节点的模板可以继续直接输出 PDF。
...
模板编译结果通过 `inspection()` 提供不可变静态契约，包括全部可能分支的数据路径、include 参数、格式化器、表达式语言、节点类型、文档特性和输出白名单。宿主可以在发布或启用前核对数据上下文与目标渲染器能力；检查结果不会携带 XML 正文、表达式正文、比较值或业务数据。
