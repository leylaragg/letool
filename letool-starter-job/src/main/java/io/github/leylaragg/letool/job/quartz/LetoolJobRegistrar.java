...
public class LetoolJobRegistrar implements SmartInitializingSingleton {
    ...
    private Method findHandlerMethod(Class<?> targetClass, String jobName) {
        Map<Method, io.github.leylaragg.letool.job.annotation.JobHandler> methods =
                MethodIntrospector.selectMethods(targetClass,
                        (MethodIntrospector.MetadataLookup<io.github.leylaragg.letool.job.annotation.JobHandler>) method ->
                        AnnotatedElementUtils.findMergedAnnotation(
                                method, io.github.leylaragg.letool.job.annotation.JobHandler.class));
        if (methods.size() != 1) {
